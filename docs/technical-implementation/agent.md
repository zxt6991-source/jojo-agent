...
- 默认使用动态迭代预算：以模型可用输入容量计算 8～16 次初始软上限；每次到达软上限时，如果最近工具轮产生了新结果或仍处于无进展恢复阶段，就按 4～8 次继续扩容，直至 32～64 次安全硬上限。显式 `maxIterations` 保持固定语义。每轮注入当前剩余预算，达到最终上限后写入内部收尾原因，并额外请求一次禁用工具的最终回答；最终回答必须说明已完成、未完成和下一步，避免直接向用户暴露裸 `max_iterations` 错误。
