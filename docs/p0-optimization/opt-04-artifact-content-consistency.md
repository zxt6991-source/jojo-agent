...
> 状态：已按本方案实现（2026-09-16）。下文保留原设计描述；实际交付与验证结果见第 14 节。
...
- 新增 strict V2 contracts，统一原始字节 SHA-256、强 ETag、大小、检查时间及记录比对状态；持久化 descriptor 与版本重放保持原格式。
- 共享读取层保留路径、常规文件、20 MiB 与读取前后稳定性检查；V2 最多并发 4 次读取，超出时返回可重试错误，不积累读取队列。
- Desktop 新增 V2 main/preload 通道及可注入导出服务。保存前校验 revision，弹窗后重新校验授权和 workspace 绑定，写入同一 Buffer；拒绝源路径和同 inode 目标，并按窗口限制导出并发。
- 面板新增显式状态、刷新、stale 提示、实际大小与指纹；下载类型也先检查 metadata。按会话、文件、记录版本及请求代次隔离回调。临时 HTML 继续保存当前原始字符串。
- HTTP 新增 V2 metadata/content 路由，复用现有认证和会话证据加载。单个强 ETag 的 `If-Match` 不匹配返回 412，失败不附带文件响应头；none/custom scope 仅可读取 conversation 内容。

### 兼容与发布说明

旧 Desktop 方法和 HTTP v1 接口继续可用，其无条件保存不提供 V2 内容一致性保证。新版 renderer 要求 main/preload 同步升级，缺少 V2 方法或处理器时提示更新并禁用条件保存，不降级到旧保存。无需数据迁移。HTML 清洗、sandbox/CSP 和相对资源访问限制保持原策略。

### 验证

- 6 个 Artifact 定向 Vitest 文件：33 项通过，包含新增 `artifact-storage-stability.test.ts` 的确定性读取竞态测试。
- 全量 Vitest：1117 项通过、2 项跳过；另行补跑上述新增读取竞态测试。首次沙箱执行因本地端口监听 EPERM 失败，允许本地服务后全量重跑通过。
- `pnpm lint`、`pnpm typecheck` 通过。
- Desktop E2E 构建及 `generated-documents.spec.ts`：2 项通过，覆盖 Markdown/PDF 冲突后刷新保存、HTML 隔离、取消导出和切会话关闭面板。

本实现仍是当前内容的稳定读取与条件导出，不保存历史二进制副本，也不增加 watcher、轮询、服务器写盘 API 或新的 workspace 访问权限。
