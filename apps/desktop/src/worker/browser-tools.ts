...
const ACTIONS = {
  ...
  browser_record_start: { schema: BrowserRecordStartInput, action: 'record_start', description: 'Start an in-memory recording of successful browser workflow steps.' },
  browser_record_stop: { schema: BrowserRecordStopInput, action: 'record_stop', description: 'Stop the active browser workflow recording.' },
  browser_recordings: { schema: BrowserRecordingsInput, action: 'recordings', description: 'List in-memory browser workflow recordings without exposing typed text.' },
  browser_replay: { schema: BrowserReplayInput, action: 'replay', description: 'Replay a recorded browser workflow with bounded recovery for clearly safe transient failures.' },
...
const TARGET_PROPERTIES = {
  selector: { type: 'string', description: 'CSS selector. Use ref from browser_read when available.' },
  ref: { type: 'string', pattern: '^e[1-9][0-9]*$', description: 'Stable element ref returned by browser_read.' }
} as const;

function targetChoice(required: boolean): Record<string, unknown> {
  return required
    ? { oneOf: [{ required: ['ref'] }, { required: ['selector'] }] }
    : { not: { required: ['ref', 'selector'] } };
}
...
function inputSchemaFor(name: BrowserToolName): Record<string, unknown> {
  ...
  if (name === 'browser_record_stop' || name === 'browser_recordings') return { type: 'object', properties: {}, additionalProperties: false };
  if (name === 'browser_replay') return {
    type: 'object',
    properties: {
      recordingId: { type: 'string', pattern: '^r[1-9][0-9]*$' },
  ...
  if (name === 'browser_wait') return {
    ...
    ...targetChoice(true), additionalProperties: false
  };
  if (name === 'browser_scroll') return {
    type: 'object',
    properties: {
      ...TARGET_PROPERTIES,
    ...
    ...targetChoice(false), additionalProperties: false
  };
  if (name === 'browser_click') return { type: 'object', properties: TARGET_PROPERTIES, ...targetChoice(true), additionalProperties: false };
  if (name === 'browser_type') return {
    type: 'object', properties: { ...TARGET_PROPERTIES, text: { type: 'string' }, submit: { type: 'boolean', default: false } },
    required: ['text'], ...targetChoice(true), additionalProperties: false
  };
  if (name === 'browser_press') return {
    type: 'object',
    properties: { ...TARGET_PROPERTIES, key: { type: 'string', minLength: 1, maxLength: 32 } },
    required: ['key'], ...targetChoice(false), additionalProperties: false
  };
  if (name === 'browser_select') return {
    type: 'object',
    properties: {
      ...TARGET_PROPERTIES,
      values: { type: 'array', items: { type: 'string', maxLength: 1000 }, minItems: 1, maxItems: 20 }
    },
    required: ['values'], ...targetChoice(true), additionalProperties: false
  };
  if (name === 'browser_upload') return {
    type: 'object',
    properties: {
      ...TARGET_PROPERTIES,
      paths: { type: 'array', items: { type: 'string', minLength: 1, maxLength: 4096 }, minItems: 1, maxItems: 10 }
    },
    required: ['paths'], ...targetChoice(true), additionalProperties: false
  };
...
export class BrowserToolBridge {
  ...
  constructor(private readonly post: BrowserRequestPost, private readonly settings: () => { enabled: boolean; allowedDomains: string[] }) {}
...
export class BrowserPermissionGate implements PermissionGate {
  constructor(private readonly base: PermissionGate, private readonly settings: () => { enabled: boolean; allowedDomains: string[] }) {}
  ...
  async check(call: ToolCall, context: { sessionId: string; workingDirectory: string }): Promise<PermissionDecision> {
    ...
    if (['pages', 'select_page', 'record_stop', 'recordings', 'read', 'wait', 'scroll', 'back', 'reload', 'screenshot', 'downloads', 'console', 'network', 'errors'].includes(action.action)) return { decision: 'allow' };
    ...
    else if (action.action === 'record_start') reason = 'Start an in-memory browser workflow recording; typed text will be retained until the app exits';
