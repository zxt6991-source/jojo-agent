...
export type BrowserElementMatch = {
...
export function scoreBrowserElementCandidate(
...
export function chooseBrowserElementCandidate(
  ...
  const ranked = candidates
    .map((candidate) => ({ candidate, score: scoreBrowserElementCandidate(fingerprint, candidate) }))
    .filter((entry) => Number.isFinite(entry.score))
    .sort((left, right) => right.score - left.score);
  const best = ranked[0];
  if (!best || best.score < 35) return { score: best?.score ?? Number.NEGATIVE_INFINITY, ambiguous: false };
  const second = ranked[1];
  if (second && second.score >= best.score - 8) return { score: best.score, ambiguous: true };
...
export type BrowserKeyDefinition = {
  key: string;
  code: string;
